...
class CustomTextEditState extends State<CustomTextEdit> with TextInputClient {
  ...
  void updateEditingValue(TextEditingValue value) {
    // Issue #201: Detect deletion from AOSP keyboards that modify text
    // directly without sending a hardware key event.
    if (value.composing.isCollapsed &&
        value.text.length < _currentEditingState.text.length) {
      _currentEditingState = value;
      widget.onDelete();
      // Reset editing state after handling delete
    ...
    if (_currentEditingState.text.length < _initEditingState.text.length) {
      ...
      // Issue #164: Skip re-sending text that was already handled during
      // Korean IME composition.
      if (textDelta.isNotEmpty) {
        if (_lastComposingText != null && textDelta == _lastComposingText) {
          // Already sent during composition, skip.
        } else {
